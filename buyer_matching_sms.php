...
if(isset($_POST['action']) && $_POST['action']=="get_lead_details_sms_MD"){
    // if($_POST['lead_id'] > 0){
        ...
        $leadquery = "SELECT mobile FROM contact WHERE  mobile IS NOT NULL" ;
        ...
        <form method="post" action="" id="form_sms">
            ...
            <?php foreach($contactNumbers as $number ) { ?>
              <input type="hidden" value="<?= $number ?>" name="to_phone[]" class="to_phone" />
            <?php } ?>
        ...
        <script>
          ...
          jQuery("#send_sms_btn").on('click',function(e){
            ...
            var to_phone = [];
            $(".to_phone").each((index, ele) => {
              to_phone.push($(ele).val());
            });
