...
if(isset($_POST['action']) && $_POST['action']=="get_lead_details_email_MD"){
    // if($_POST['lead_id'] > 0){
        ...
        if(count($contact_ids)) {
            ...
            $leadquery = "SELECT email FROM contact WHERE  email IS NOT NULL" ;
            if(count($contact_ids)) {
                $leadquery .= " AND `id` IN (" . implode(',', $contact_ids) . ") ";
            }

            $s_f_l = getRows(mysqli_query($sqlConnect, $leadquery));

            $contactNumbers = array_map(function($contact) {
            ...
            ?>   
                ...
                <form method="post" action="" id="form_email" enctype="multipart/form-data">
                    ...
                    <?php foreach($contactNumbers as $email ) { ?>
                    <input type="hidden" value="<?= $email ?>" name="to_email[]" class="to_email" />
                    <?php } ?>
                ...
                jQuery("#send_email_btn").on('click',function(e){
                    ...
                    var to_email = [];
                    $(".to_email").each((index, ele) => {
                        to_email.push($(ele).val());
                    });
