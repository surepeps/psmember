...
if($action == 'verify_user'){
    ...
    $message = null;
    if($username) {
        $user = getTableData(T_USERS, [
            'username' => $username
        ], 1);
        if($user) {
            $message = "User is already found with this username, please try again with another username";
        }
    }
    ...
    if(empty($message)) {
        ...
        if($user) {
            $message = "User is already found with this email, please try again with another email";
        }
    }
    ...
    $re_data  = array(
        ...
        'birthday' => '0000-00-00',
        'pro_type' => $pro_type_default
    ); /*dvm added is_pro pro_type*/
    ...
    if (empty($_POST['phone_num'])) {
        ...
        if($ref = filter('ref')){
            $user = getRow("select user_id FROM " . T_USERS . " WHERE ". getWhere(['username' => filter('username')]) );
            if($user && $ref) {
                $refSaved = saveRefferenceUser($user, $ref);
                if(!$refSaved) {
                    $register = false;
                    $message = "Error while making the referrence";
                }
    ...
    if ($register === true) {
        if ($activate == 1) {
            ...
            setcookie("email_phone", $_POST['phone_num'], time()+3600);
            //if ($query) {
                if (Wo_SendSMSMessage($_POST['phone_num'], $message) === true) {
                    $register = Wo_RegisterUser($re_data, $in_code);
                    $user_id           = Wo_UserIdFromUsername($_POST['username']);
                    $_SESSION['user_id_dvm'] = $user_id; /*dvm mods*/ 
                    setcookie("user_id_dvm", $user_id, time()+3600);
                    setcookie("email_dvm", $_POST['email'], time()+3600);
                    
                    setcookie("l_code", $code, time()+3600);
                    $query             = mysqli_query($sqlConnect, "UPDATE " . T_USERS . " SET `sms_code` = '{$random_activation}' WHERE `user_id` = {$user_id}");
                    $data = array(
                        'status' => 300,
                        'location' => Wo_SeoLink('index.php?link1=confirm-sms?code=' . $code),														'location_url' => Wo_SeoLink('index.php?link1=confirm-sms?code=' . $code), /*dvm mods*/
                    );
                    if ($wo['config']['membership_system'] == 1) {
                        $session             = Wo_CreateLoginSession(Wo_UserIdFromUsername($_POST['username']));
                        $_SESSION['user_id'] = $session;
                        ...
                        $f=fopen("session.txt","w");
                        fwrite($f,$_SESSION['user_id_dvm']."---".$user_id);
                        fclose($f);
                    }
                } else {
