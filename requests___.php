...
if($f=="subscription_active") {

    $responsetype = "subscription_active";
    $responsedata = json_decode(file_get_contents("php://input"),1);


   // $mysqli = mysqli_query($sqlConnect, "INSERT INTO `send_owl_responses` VALUES ('','".$responsetype."','".$responsedata."')");
    ...
    if(!empty($responsedata)) {
        ...
        try {
            $packageid = $responsedata['order']['cart']['cart_items'][0]['product']['id'];
            $transactionid =  $responsedata['order']['transactions'][0]['gateway_transaction_id'];
            $packagetype = "";
            $newesarr['packageid'] = $packageid;
            switch ($packageid) {
                case '10000':
                    $packagetype = "pro";
                    $packageid = 3;
            ...
            $query_one = "SELECT user_id FROM " . T_USERS . " WHERE email = '".$responsedata['order']['buyer_email']."'";
            ...
            $amount = $responsedata['order']['transactions'][0]['net_price'];

            $next_payment_date = $responsedata['order']['next_payment_date'];
            ...
            $query3 = mysqli_query($sqlConnect, "INSERT INTO `Wo_Package_upgrade_transactions` (`user_id`, `price `,`transaction_date`, `package_id`, `transaction_id`,`next_payment_date`) VALUES ({$userid}, {$amount}, '{$date}', '".$packageid."','{$transactionid}','{$next_payment_date}')");
...
if ($f == 'register') {
    ...
    if (empty($errors)) {
        ...
        if (!empty($_SESSION['user_id']) && !empty($field_data)) {
            ...
            $insert  = Wo_UpdateUserCustomData($_SESSION['user_id'], $field_data, false);
        }
    }
    header("Content-type: application/json");
    if (isset($errors)) {
            ...
            if (!empty($Userdata['user_id'])) {
                ...
                if (!empty($_POST['pro_type']) && (Wo_IsAdmin() || Wo_IsModerator())) {
                    ...
                    if ($_POST['pro_type'] == 'free') {
                        ...
                        $packagepoints = Wo_package_stratistic_points(2);
                        $Strasticpoint = $Strasticpoint + $packagepoints;
                    } else if ($_POST['pro_type'] == 'pro') {
                        $member_type = 3;
                        $member_pro  = 1;
                        $time        = time();
                        $packagepoints = Wo_package_stratistic_points(3);
                        $Strasticpoint = $Strasticpoint + $packagepoints;
                    } else if ($_POST['pro_type'] == 'advanced') {
                        $member_type = 4;
                        $member_pro  = 1;
                        $time        = time();
                        $packagepoints = Wo_package_stratistic_points(4);
                        $Strasticpoint = $Strasticpoint + $packagepoints;
                    } else if ($_POST['pro_type'] == 'ultima') {
                        $member_type = 5;
                        $member_pro  = 1;
                        $time        = time();
                        $packagepoints = Wo_package_stratistic_points(5);
                        $Strasticpoint = $Strasticpoint + $packagepoints;
...
if ($f == "update_email_settings") {
    if (isset($_POST['user_id']) && Wo_CheckSession($hash_id) === true) {
        ...
        $e_email_makeoffer = 0;  
        $e_email_counteroffer = 0; 
        $e_email_rejectoffer = 0;
        $e_email_acceptoffer = 0;
        $e_email_schedulevisit = 0;
        $e_email_acceptvisit = 0;
        $e_email_rejectvisit = 0;
        $e_email_reschedulevisit = 0;
        $e_email_matchcriteria = 0;
        ...
        if (!empty($_POST['e_email_schedulevisit'])) {
            if (in_array($_POST['e_email_schedulevisit'], $array)) {
                $e_email_schedulevisit = 1;
            }
        }


        if (!empty($_POST['e_email_makeoffer'])) {
            if (in_array($_POST['e_email_makeoffer'], $array)) {
                $e_email_makeoffer = 1;
            }
        }

        if (!empty($_POST['e_email_counteroffer'])) {
            if (in_array($_POST['e_email_counteroffer'], $array)) {
                $e_email_counteroffer = 1;
            }
        }

        if (!empty($_POST['e_email_rejectoffer'])) {
            if (in_array($_POST['e_email_rejectoffer'], $array)) {
                $e_email_rejectoffer = 1;
            }
        }

        if (!empty($_POST['e_email_acceptoffer'])) {
            if (in_array($_POST['e_email_acceptoffer'], $array)) {
                $e_email_acceptoffer = 1;
            }
        }


        if (!empty($_POST['e_email_acceptvisit'])) {
            if (in_array($_POST['e_email_acceptvisit'], $array)) {
                $e_email_acceptvisit = 1;
            }
        }

        if (!empty($_POST['e_email_rejectvisit'])) {
            if (in_array($_POST['e_email_rejectvisit'], $array)) {
                $e_email_rejectvisit = 1;
            }
        }

        if (!empty($_POST['e_email_reschedulevisit'])) {
            if (in_array($_POST['e_email_reschedulevisit'], $array)) {
                $e_email_reschedulevisit = 1;
            }
        }

        if (!empty($_POST['e_email_matchcriteria'])) {
            if (in_array($_POST['e_email_matchcriteria'], $array)) {
                $e_email_matchcriteria = 1;
            }
        }

        if (!empty($_POST['e_notify_sentme_msg'])) {
            if (in_array($_POST['e_notify_sentme_msg'], $array)) {
                $e_notify_sentme_msg = 1;
            }
        }

         if (!empty($_POST['e_notify_sentme_sms'])) {
            if (in_array($_POST['e_notify_sentme_sms'], $array)) {
                $e_notify_sentme_sms = 1;
            }
        }

         if (!empty($_POST['e_email_sentme_sms'])) {
            if (in_array($_POST['e_email_sentme_sms'], $array)) {
                $e_email_sentme_sms = 1;
            }
        }
        ...
        $arrayvals = array(
            'e_email_makeoffer' => $e_email_makeoffer,
            'e_email_counteroffer' => $e_email_counteroffer,
            'e_email_rejectoffer' =>$e_email_rejectoffer,
            'e_email_acceptoffer' =>$e_email_acceptoffer, 
            'e_email_schedulevisit' => $e_email_schedulevisit,
            'e_email_acceptvisit' => $e_email_acceptvisit, 
            'e_email_rejectvisit' =>  $e_email_rejectvisit,
            'e_email_reschedulevisit' => $e_email_reschedulevisit, 
            'e_email_matchcriteria' => $e_email_matchcriteria);

        $prevdata = array();
        if (!empty($wo['user']['notification_settings'])) {
             $prevdata = unserialize(html_entity_decode($wo['user']['notification_settings']));
        }

        $prevdata['e_email_makeoffer'] = $e_email_makeoffer;
        $prevdata['e_email_counteroffer'] = $e_email_counteroffer;
        $prevdata['e_email_rejectoffer'] = $e_email_rejectoffer;
        $prevdata['e_email_acceptoffer'] = $e_email_acceptoffer;
        $prevdata['e_email_schedulevisit'] = $e_email_schedulevisit;
        $prevdata['e_email_acceptvisit'] = $e_email_acceptvisit;
        $prevdata['e_email_rejectvisit'] = $e_email_rejectvisit;
        $prevdata['e_email_reschedulevisit'] = $e_email_reschedulevisit;
        $prevdata['e_email_matchcriteria'] = $e_email_matchcriteria;
        $prevdata['e_notify_sentme_msg'] = $e_notify_sentme_msg;
        $prevdata['e_email_sentme_sms'] = $e_email_sentme_sms;
        $prevdata['e_notify_sentme_sms'] = $e_notify_sentme_sms;
        
        

         $Update_data = serialize($prevdata);
...
if ($f == "update_notifications_settings") {
    ...
    if (isset($_POST['user_id']) && Wo_CheckSession($hash_id) === true) {
        ...
        $array               = array(
         ...
         if (!empty($_POST['e_schedulevisit'])) {
            if (in_array($_POST['e_schedulevisit'], $array)) {
                $e_schedulevisit = 1;
            }
        }

        if (!empty($_POST['e_makeoffer'])) {
            if (in_array($_POST['e_makeoffer'], $array)) {
                $e_makeoffer = 1;
            }
        }

        if (!empty($_POST['e_counteroffer'])) {
            if (in_array($_POST['e_counteroffer'], $array)) {
                $e_counteroffer = 1;
            }
        }

        if (!empty($_POST['e_rejectoffer'])) {
            if (in_array($_POST['e_rejectoffer'], $array)) {
                $e_rejectoffer = 1;
            }
        }

        if (!empty($_POST['e_acceptoffer'])) {
            if (in_array($_POST['e_acceptoffer'], $array)) {
                $e_acceptoffer = 1;
            }
        }

        if (!empty($_POST['e_acceptvisit'])) {
            if (in_array($_POST['e_acceptvisit'], $array)) {
                $e_acceptvisit = 1;
            }
        }

        if (!empty($_POST['e_rejectvisit'])) {
            if (in_array($_POST['e_rejectvisit'], $array)) {
                $e_rejectvisit = 1;
            }
        }

        if (!empty($_POST['e_reschedulevisit'])) {
            if (in_array($_POST['e_reschedulevisit'], $array)) {
                $e_reschedulevisit = 1;
            }
        }

        if (!empty($_POST['e_matchcriteria'])) {
            if (in_array($_POST['e_matchcriteria'], $array)) {
                $e_matchcriteria = 1;
            }
        }
        ...
        $Update_data = array(
            ...
            'e_visited' => $e_visited,
            'e_profile_wall_post' => $e_profile_wall_post,
            'e_makeoffer' => $e_makeoffer,
            'e_counteroffer' => $e_counteroffer,
            'e_rejectoffer' =>$e_rejectoffer, 
            'e_acceptoffer' =>$e_acceptoffer, 
            'e_schedulevisit' => $e_schedulevisit,
            'e_acceptvisit' => $e_acceptvisit, 
            'e_rejectvisit' =>  $e_rejectvisit,
            'e_reschedulevisit' => $e_reschedulevisit, 
            'e_matchcriteria' => $e_matchcriteria
        );
        $prevdata = array();
        if (!empty($wo['user']['notification_settings'])) {
             $prevdata = unserialize(html_entity_decode($wo['user']['notification_settings']));
        ...
        $prevdata['e_liked'] = $e_liked;
        $prevdata['e_shared'] = $e_shared;
        $prevdata['e_wondered'] = $e_wondered;
        $prevdata['e_commented'] = $e_commented;
        $prevdata['e_followed'] = $e_followed;
        $prevdata['e_accepted'] = $e_accepted;
        $prevdata['e_mentioned'] = $e_mentioned;
        $prevdata['e_joined_group'] = $e_joined_group;
        $prevdata['e_liked_page'] = $e_liked_page;
        $prevdata['e_visited'] = $e_visited;
        $prevdata['e_profile_wall_post'] = $e_profile_wall_post;
        $prevdata['e_makeoffer'] = $e_makeoffer;
        $prevdata['e_counteroffer'] = $e_counteroffer;
        $prevdata['e_rejectoffer'] =$e_rejectoffer; 
        $prevdata['e_acceptoffer'] =$e_acceptoffer; 
        $prevdata['e_schedulevisit'] = $e_schedulevisit;
        $prevdata['e_acceptvisit'] = $e_acceptvisit; 
        $prevdata['e_rejectvisit'] =  $e_rejectvisit;
        $prevdata['e_reschedulevisit'] = $e_reschedulevisit; 
        $prevdata['e_matchcriteria'] = $e_matchcriteria;
...
if ($f == 'messages') {
    ...
    if ($s == 'send_message') {
        ...
        /********************** First check if users connected other wise return *************************/
       // Wo_GetFollowers($wo['user'] ['user_id'],'profile');
        $Followers =  Wo_GetFollowers($wo['user'] ['user_id'], 'profile', 12);

        //print_r( $Followers);
        $Followerids = array();
        foreach ($Followers as $SingleFollower) {

            $Followerids[] = $SingleFollower['user_id'];

        }

        
       // print_r($Followerids);
        if(in_array($_POST['user_id'], $Followerids)) {

            $straticpints = Wo_get_stratistic_points('message');

            $Userpointquery = mysqli_query($sqlConnect, "SELECT points FROM `Wo_Users` WHERE user_id=".$wo['user']['user_id']);
            ...
            if($points < $straticpints) {
                ...
                $data = array(
                        ...
                        'html' => "You do not have sufficeint Stratistic Points. ",
            ...
            $Userpointquery = mysqli_query($sqlConnect, "UPDATE `Wo_Users` SET points=".$remainingpoints." WHERE user_id=".$wo['user']['user_id']); 
...
if ($f == 'posts') {
    ...
    if ($s == 'insert_new_post') {
       ...
       /* $Pagecountquery = "SELECT count(*) as pagecount FROM `Wo_Pages` WHERE user_id=".$wo['user']['user_id'];
        $sql = mysqli_query($sqlConnect, $Pagecountquery);
        $fetched_data = mysqli_fetch_assoc($sql);
        if(!empty($fetched_data))
            $pagecount = $fetched_data['pagecount'];

        $pagepermisson = Wo_Package_permission_Count('create_posts'); 

        if($pagecount >= $pagepermisson && $wo['user']['username']!="admin" ) { 

            header("Content-type: application/json");
            echo json_encode(array(
                'status' => 400,
                'errors' => "You have exceeded the maximum limit of creating Posts."
            ));
            return false;
            exit();
        }*/
...
if ($f == 'stripe_payment_wallet') {
    ...
    try {
        ...
        //   ]],
        //   'mode' => 'payment',
        //   'success_url' => 'https://dev.propertysalers.com/paysuccess.php',
        //   'cancel_url' => 'https://dev.propertysalers.com/wallet',
        // ]);
...
if ($f == 'get_paypal_url') {
    ...
    if(isset($_POST['pro_id']))
        ...
        $url = Wo_PayPal($_POST['type'], $type2,$proid);
        if (!empty($url['type'])) {
            if ($url['type'] == 'SUCCESS' && !empty($url['type'])) {

                /*********************************  Add strastic points in user account according to pakage ***************************************/

                $query = mysqli_query($sqlConnect, "SELECT `stastic_point` FROM Wo_Package_permission WHERE `id` = " . $proid);
                if (mysqli_num_rows($query) > 0) {
                    $fetched_data = mysqli_fetch_assoc($query);
                    
                    $strasticpoints = $fetched_data['stastic_point'];

                    $update_array   = array(
                        'points' => $strasticpoints,
                    );
                    $mysqli         = Wo_UpdateUserData($wo['user']['user_id'], $update_array);
                }
                $data = array(
   ...
   /* print_r($_GET);
    ...
    if (isset($_GET['success']) && $_GET['success'] == 'true') {
            ...
            if ($stop == 0) {
                ...
                $pro_type = $_GET['pro_type'];
                $payment  = Wo_custom_recurring_Agreement($_GET['token']);
