...
foreach($allactivepipelines as $aapl){
    ...
    // ALL VALUES FROM FIRST LOOP 
    
    // Get Pipeline Last Updated date and time for Cron job
    $pipeDate = $aapl['date_updated'];
    
    
    // Get each active campaign pipeline timezones
    $cp_timeZone = getTimeZoneNameSingle($aapl['pipeTimeZone']);
    ...
    $gggg = GetAllContactsfrocronJob($pipeID);
    
    // LOGS OF CRON JOBS FOR TROUBLESHOOTING.....
    if(count($gggg) > 0){
        ...
        echo "</li><b> Total Numbers of Contacts in Pipe ID:- ".$pipeID." is => ".$numCont."</b></li>";
          ...
          foreach($gggg as $g){
                ...
                echo "<ul>
                        ...
                        <li>Row ID Of:- ".$g['id']." contact ID:- ".$g['contact_id']." Ready For Movement In Pipe ID:- ".$g['pipe_id']." in Stage ID:- ".$g['stage_id']." With Step ID:- ".$g['step_id']."</li>
                        <ul>
                ...
                if( $getFirstAct > 0 && $g['action_id'] == null ) {
                    ...
                    if($getNaction > 0){
                      ...
                      updatetriggertype($pipeID,3,$g['contact_id'],$tm);
                      ...
                      if($userBal >= $getActionCost){
                        ...
                        updatetriggertype($pipeID,2,$g['contact_id'],$tm);
                        MoveContstoNextStep($sdata,$pipeID,$tm);
                      
                    }elseif($getNstage > 0){
                        
                        updatetriggertype($pipeID,1,$g['contact_id'],$tm);
        ...
        echo "<li> No Cron Job Movement for Campaign Pipeline Id:- ".$pipeID."</li>";
    }
    ...
    if($getNstep > 0){
        ...
        if($g['trigger_type'] == 2){
            ...
            if(time() >= $timeTU){
                ...
                moveContactstothenestSteps($g['id'],$pipeID,$getNstep);
        ...
        // if no more step then move to the next stage and next step in the new stage
            ...
            if($g['trigger_type'] == 1){
                ...
                if(time() >= $timeTU){
                    ...
                    moveContactstothenestStages($g['id'],$pipeID,$getNstage);
...
function MoveContacttoNextAction($g,$pipeID,$tm){
    ...
    if($g['trigger_type'] == 3){
        ...
        if(time() >= $timeTU && $confirmAct == 0){
            ...
            insertcronrecords($pipeID,$g,$tm);
         
            
            updateActionToPipeline($getNaction,$g['id'],$pipeID);
...
function updatethefirstactioninstep($g,$pipeID,$tm){
    ...
    updateActionToPipeline($getFirstAct,$g['id'],$pipeID);
...
function getTimeConversion($stepD){
    ...
    // return strtotime($stepD['last_updt']. '+ '.$tm.' seconds');
}
